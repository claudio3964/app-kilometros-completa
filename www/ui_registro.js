...
const BASES_DISPONIBLES = [
  ...
  'Colonia','Chuy','La Pedrera','La Paloma',
  'Aguas Dulces','Otro'
];
...
function cargarSelectorOrigen(baseSeleccionada){
  ...
  if(!sel) return;
  sel.innerHTML = BASES_DISPONIBLES.map(b =>
    `<option value="${b}" ${b === baseSeleccionada ? 'selected' : ''}>${b}</option>`
  ).join('');
}
...
async function registrarChofer(){
  try {
    ...
    const checkRes = await fetch(
      `${SUPABASE_URL_REG}/rest/v1/choferes?empresa_id=eq.cot&legajo=eq.${legajo}&select=id,legajo,device_id`,
      {
        headers: {
    ...
    if(choferes.length > 0){
      ...
      if(choferExistente.device_id && choferExistente.device_id !== deviceId){
        ...
        await fetch(`${SUPABASE_URL_REG}/rest/v1/registro_alertas`, {
          method: "POST",
          headers: {
    ...
    const insertRes = await fetch(
      `${SUPABASE_URL_REG}/rest/v1/choferes?on_conflict=empresa_id,legajo`,
      {
        method: "POST",
        headers: {
          "Content-Type":  "application/json",
