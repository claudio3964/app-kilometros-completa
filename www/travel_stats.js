...
function saveTravelStats(stats) {
  localStorage.setItem(TRAVEL_STATS_KEY, JSON.stringify(stats));
}
...
function registrarEstadisticaViaje(travel) {
  ...
  const velocidad = travel.kmEmpresa ? travel.kmEmpresa / (duracion / 60) : 0;
  ...
  s.velocidadPromedio = (s.velocidadPromedio * (s.totalViajes - 1) + velocidad) / s.totalViajes;
...
function obtenerDuracionPromedio(origen, destino) {
  ...
  return Math.floor(stat.totalMinutos / stat.totalViajes);
}
